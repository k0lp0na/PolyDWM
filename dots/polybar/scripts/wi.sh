...
if ! command -v nmcli &> /dev/null; then
    echo "󰖪 off"  # Wi-Fi disabled icon as fallback
    exit 0
fi
...
if [[ "$wifi_status" == "disabled" ]]; then
    echo "󰖪 off"  # Wi-Fi disabled icon
    exit 0
fi
...
if [[ "$connection_status" != "full" && "$connection_status" != "limited" ]]; then
    echo "$signal_icon None"  # Disconnected icon with "none"
    exit 0
fi
# Get the SSID
ssid=$(nmcli -t -f IN-USE,SSID dev wifi | grep '^*' | cut -d: -f2)
if [ -z "$ssid" ]; then
    ssid="none"  # No SSID available
fi


# Check for packet loss
if ! ping -c 1 -W 1 8.8.8.8 &> /dev/null; then
    echo "󰤫 $ssid"  # Packet loss icon
    exit 0
fi



# Output the signal icon and SSID
